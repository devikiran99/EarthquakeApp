...
final shouldShowLoadingProvider = StateProvider((ref) => false);
...
class QueryParamsProvider extends Notifier<QueryParams> {
  ...
  QueryParams build() {
    ...
    final startTime = getFormattedDateTime(DateTime
        .now()
        .subtract(const Duration(days: 1))
        .millisecondsSinceEpoch);
    final endTime = getFormattedDateTime(DateTime
        .now()
        .millisecondsSinceEpoch);
  ...
  Future<void> setLocation(bool value) async{
    ...
    if(value){
      ref.read(shouldShowLoadingProvider.notifier).state = true;
      final position = await determinePosition();
      final latitude = position.latitude;
      final longitude = position.longitude;
      ref.read(cityProvider.notifier).state = await getCurrentCity(latitude, longitude);
      ref.read(shouldShowLoadingProvider.notifier).state = false;
